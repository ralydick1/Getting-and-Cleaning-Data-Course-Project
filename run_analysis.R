#run_analysis.R
#Load packages
library(data.table)
library(dplyr)

#Download UCI data files from the web, unzip them, and specify time
URL <- "https://d396qusza40orc.cloudfront.net/getdata%2Fprojectfiles%2FUCI%20HAR%20Dataset.zip"
destFile <- "ProjectDataset.zip"
if (!file.exists(destFile)){
  download.file(URL, destfile = destFile, mode='wb')
}
if (!file.exists("./UCI HAR Dataset")){
  unzip(destFile)
}

## 1. Import and Merge all Data
  ## 1.1 Read Data
  ActivityTest <- read.table("./UCI HAR Dataset/test/y_test.txt", header = F)
  ActivityTrain <- read.table("./UCI HAR Dataset/train/y_train.txt", header = F)
  ActivityLabels <- read.table("./UCI HAR Dataset/activity_labels.txt", header = F)

  FeaturesTest <- read.table("./UCI HAR Dataset/test/X_test.txt", header = F)
  FeaturesTrain <- read.table("./UCI HAR Dataset/train/X_train.txt", header = F)
  FeaturesNames <- read.table("./UCI HAR Dataset/features.txt", header = F)

  SubjectTest <- read.table("./UCI HAR Dataset/test/subject_test.txt", header = F)
  SubjectTrain <- read.table("./UCI HAR Dataset/train/subject_train.txt", header = F)

  ## 1.2 Merge and Pivot dataframes: Features Test&Train,Activity Test&Train, Subject Test&Train
  FeaturesData <- rbind(FeaturesTest, FeaturesTrain)
  SubjectData <- rbind(SubjectTest, SubjectTrain)
  ActivityData <- rbind(ActivityTest, ActivityTrain)

  ## 1.3 Renaming colums in ActivityData & ActivityLabels dataframes
  names(ActivityData) <- "ActivityN"
  names(ActivityLabels) <- c("ActivityN", "Activity")

  ## 1.4 Join Activity data
  Activity <- left_join(ActivityData, ActivityLabels, "ActivityN")[, 2]

  ## 1.5 Rename Subject and Features Columns
  names(SubjectData) <- "Subject"
  names(FeaturesData) <- FeaturesNames$V2

## 1.6. Create one large Dataset with only these variables: SubjectData,  Activity,  FeaturesData
  DataSet <- cbind(SubjectData, Activity)
  DataSet <- cbind(DataSet, FeaturesData)

## 2. Create New datasets by extracting only the measurements on the mean and standard deviation for each measurement
  subFeaturesNames <- FeaturesNames$V2[grep("mean\\(\\)|std\\(\\)", FeaturesNames$V2)]
  DataNames <- c("Subject", "Activity", as.character(subFeaturesNames))
  DataSet <- subset(DataSet, select=DataNames)

## 3-4. Rename the columns of the large dataset using more descriptive activity names
  names(DataSet) <- gsub("^t", "time", names(DataSet))
  names(DataSet) <- gsub("^f", "frequency", names(DataSet))
  names(DataSet) <- gsub("Acc", "Accelerometer", names(DataSet))
  names(DataSet) <- gsub("Gyro", "Gyroscope", names(DataSet))
  names(DataSet) <- gsub("Mag", "Magnitude", names(DataSet))
  names(DataSet) <- gsub("BodyBody", "Body", names(DataSet))

## 5. Create a second, independent tidy data set with the average of each variable for each activity and each subject
  # 5.1 Making a second Tidy Data Set
  SecondDataSet <- aggregate(. ~Subject + Activity, DataSet, mean)
  SecondDataSet <- SecondDataSet[order(SecondDataSet$Subject,SecondDataSet$Activity),]

  # 5.2 Save this tidy dataset to local file
  write.table(SecondDataSet, file = "tidydata.txt",row.name=FALSE)